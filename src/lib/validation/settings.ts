 ...
 * A single entry today, but kept as an array (rather than inlining the
 * string literal at the call site) so a second setting only needs adding
 * here plus a new schema/case in the action's switch. */
export const ALLOWED_SETTING_KEYS = ["quote.validityDays"] as const;
